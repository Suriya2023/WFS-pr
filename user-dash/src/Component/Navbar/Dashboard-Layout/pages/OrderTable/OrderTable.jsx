...
import { XCircle, CheckCircle, Download, FileText, Package, Search } from 'lucide-react';
                                ...
                                {data.length === 0 ? (
                                    ...
                                    data.map((order, index) => (
                                        <tr key={index} className="border-b border-gray-200 hover:bg-gray-50 dark:border-gray-800 dark:hover:bg-gray-800/50 transition-colors">
                                            ...
                                            <td className="px-4 py-3">
                                                <span className={`px-2 py-1 text-[10px] font-bold uppercase rounded-full ${order.status === 'pending_payment' ? 'bg-amber-100 text-amber-700' :
                                                        order.status === 'paid' ? 'bg-green-100 text-green-700' :
                                                            order.status === 'draft' ? 'bg-slate-100 text-slate-600' :
                                                                'bg-blue-100 text-blue-600'
                                                    }`}>
                                                    {order.status === 'pending_payment' ? 'Pending Amount' : order.status}
                                                </span>
                                            </td>
                                                ...
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    {(order.status === 'pending_payment' || order.status === 'payment_pending') && onPayNow && (
                                                        <button
                                                            ...
                                                            className="px-3 py-1.5 bg-green-600 text-white rounded-lg text-xs font-bold hover:bg-green-700 transition shadow-sm uppercase tracking-wider whitespace-nowrap flex items-center gap-1"
                                                        >
                                                    ...
                                                    {(order.status === 'draft' || order.status === 'payment_pending' || order.status === 'paid') ? (
                                                        <button
                                                            ...
                                                            className="px-3 py-1.5 bg-orange-500 text-white rounded-lg text-xs font-bold hover:bg-orange-600 transition shadow-sm uppercase tracking-wider"
                                                        >
                                                            Edit
                                                        </button>
                                                    ) : (
                                                        <button
                                                            ...
                                                            className="text-blue-600 hover:text-blue-700 text-sm font-medium dark:text-blue-400 dark:hover:text-blue-300 whitespace-nowrap"
                                                        >
                                                            View
                                                        </button>
                                                    )}
